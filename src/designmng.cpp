...
bool DesignMng::saveProjectConfig(string filename, string project_name){
	...
	file_out << "set topcell " + circuit->getTopCell() << "\n";
	...
	if (words.size() != 0){
		switch (getCommandCode(words)){
			...
			case EXPORT_LAYOUT:
				{
					...
					words[2]=upcase(words[2]);
					if(circuit->getLayout(words[2])){
						if(tipo=="GDS"){
							...
							SaveLayout slayout;
							slayout.saveCif(removeExt(filename)+".cif", *rules, *(circuit->getLayouts()), words[2]);
							slayout.cif2Cadence(name,  words[2]);
							ret=1;
						}else if(tipo=="CIF"){
							SaveLayout slayout;
							slayout.saveCif(removeExt(filename)+".cif", *rules, *(circuit->getLayouts()), words[2]);
							...
							SaveLayout slayout;
							slayout.saveSight(removeExt(filename)+".sight", *rules, *(circuit->getLayouts()), words[2]);
							ret=1;
			...
			case PLACE_TERMINALS:
				...
				break;
			case PLACE_TERMINALS_OPTIMIZE:
				ret=placer->optimizeInterfaceTerminals();
				break;
			case PLACE_FPLACE:
				{
			...
			/****  CELLGEN - 9  ****/
			case CELLGEN_SETNRTRACKS:
				autocell->setNrTracks(atoi(words[3].c_str()));
				ret=1;
				break;

			case CELLGEN_SETPOSTRACKS:
				autocell->setPosTracks(atoi(words[3].c_str()));
				ret=1;
				break;
