...
void CellNetlst::multiply(int m){
...
bool CellNetlst::calcIOs( string gndName, string vddName ){
	string vdd = this->getNet(vddName).name;
	string gnd = this->getNet(gndName).name;

	
	for(int cnt=0; cnt<inouts.size(); ++cnt){
	    int gate=0;
	    int diff=0;
    	for(list<t_net2>::iterator trans_it=nets[inouts[cnt]].trans.begin(); trans_it!=nets[inouts[cnt]].trans.end(); ++trans_it){
    	    //cout << "TESTE: " << trans_it->type << endl;
    	    if (trans_it->type == GATE){
                gate++;
    	    }//end if
    	    else if (trans_it->type == DRAIN || trans_it->type == SOURCE)
                diff++;
        }//end for
        //cout << "Diff = " << diff << " Gate = " << gate << endl;



		inouts_type[cnt] = IOTYPE_INOUT;
        if ( nets[inouts[cnt]].name != gnd && nets[inouts[cnt]].name != vdd ) {
				if (gate && !diff ){
				inouts_type[cnt] = IOTYPE_INPUT;
			}//end if
			else if (!gate && diff){
				inouts_type[cnt] = IOTYPE_OUTPUT;
			}// end if
		} // end if

        //cout << "Type pin: " << inouts_type[cnt] << endl;
	} // end for
...
void CellNetlst::insertInstance(string instName, string subCircuit, vector<string>& ports, int multiplyer){
	Inst tmp;
	tmp.m=multiplyer;
...
*/	cout << "Transistors: " << endl;
	...
	for(map<string,Inst>::iterator tmp=instances.begin(); tmp!=instances.end(); ++tmp){
		...
		cout << tmp->second.subCircuit << " M=" << tmp->second.m << endl;
	}
};
...
Inst &CellNetlst::getInstance( string name ) {
    map<string,Inst>::iterator it = instances.find( name );
    return it->second;
}

t_net& CellNetlst::getNet( string name ) {
...
void CellNetlst::calculateInputCapacitances() {
    double cinN = 0;
	double cinP = 0;
	string technology = "45nm";
    //values for 350nm technology
	float cinTechnologyP = 5.2219E-16 * 1.6; //*1.6, pois é a relação P/N para a tecnologia 350n 
    ...
    double cinTechnologyP_45 = 15.9431E-16; //using liberty values
	//float cinTechnologyN_45 = 8.8979E-17;
	double cinTechnologyN_45 = 15.9431E-16; //using liberty values

    //cout << "\n\n\n";
    //cout << this->getName() << "\n";

    for(int c=0; c<inouts.size(); c++){
	//cout << "\tPin Name: " << nets[inouts[c]].name << endl;
	//cout << "\tPin Type: " << IOTypeName[inouts_type[c]] << endl;

	if(inouts_type[c] == IOTYPE_INPUT){
	    for (int j=0; j< trans.size(); j++){
		//cout << "\t\t" << trans[j].name << "\t"<< TransistorTypeName[trans[j].type] << "\t" << nets[trans[j].gate].name << endl;

		if ( trans[j].gate != inouts[c] )
		    continue;

		if (trans[j].type == PMOS )
		    cinP += trans[j].width;
		else if (trans[j].type == NMOS)
		    cinN += trans[j].width;
		else
		    continue;
	    } // end for
